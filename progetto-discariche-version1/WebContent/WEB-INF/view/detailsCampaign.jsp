...
<body>
	...
	<div id="event">
		...
		<c:if test="${not empty requestScope.ListLocality}">
			...
			<c:forEach var="element" items="${requestScope.ListLocality}">
				<br>
				<tr>
					<td>nome:<c:out value="${element.name}"/>
					</td>

					<td>town:<c:out value="${element.town}"/>
					</td>

					<td>region:<c:out value="${element.region}"/>
					</td>
					<br>
				</tr>
	...
	<form action="startCampaign" method="POST">
		<input type="hidden" name="campaignid"
			value="<c:out value = "${Campaign.id}"/>" /> <input type="submit"
			value="Avvia campagna" />
	</form>
	<form action="closeCampaign" method="POST">
		<input type="hidden" name="campaignid"
			value="<c:out value = "${Campaign.id}"/>" /> <input type="submit"
			value="Chiudi campagna" />
	</form>
	<a href="modifyProfile">Modify Profile</a>
	<a name="campaignid"
		href='showStatistics?campaignid=<c:out value ="${Campaign.id}" />'>visualize
		statistics</a>
	<a href="showHome">Back to home </a>
	<a href="showMap?campaignid=<c:out value ="${Campaign.id}" />">showMap</a>
